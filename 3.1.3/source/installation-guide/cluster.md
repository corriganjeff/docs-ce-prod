...
- On all of the non-primary gluu cluster members (not the nginx server), [download the Gluu packages](https://gluu.org/docs/ce/3.1.3/installation-guide/install/) but **don't run `setup.py` yet** on the non-primary nodes!

- On the primary gluu-server, login to the chroot and cd to `/install/community-edition-setup/`

- After setup was completed on the primary server a file named "setup.properties.last" was created in the same directory. We want to copy the `/install/community-edition-setup/setup.properties.last` file from the first install to the other servers as `setup.properties` in order to maintain the same configuration across the nodes. (Here I have SSH access to my other server outisde the Gluu chroot)
